...
#define QF_OS_OBJECT_TYPE    GCond
...
#define QF_INT_DISABLE()     g_mutex_lock(&QF_pThreadMutex_)
#define QF_INT_ENABLE()      g_mutex_unlock(&QF_pThreadMutex_)
...
#include <glib.h>   /* POSIX-thread API */
...
extern GMutex QF_pThreadMutex_; /* mutex for QF critical section */
...
/* interface used only inside QF implementation, but not in applications */
    ...
    #define QACTIVE_EQUEUE_WAIT_(me_) \
        while ((me_)->eQueue.frontEvt == (QEvt *)0) \
            g_cond_wait(&(me_)->osObject, &QF_pThreadMutex_)
    #define QACTIVE_EQUEUE_SIGNAL_(me_) \
        Q_ASSERT_ID(410, QF_active_[(me_)->prio] != (QActive *)0); \
        g_cond_signal(&(me_)->osObject)
